...
class TestPieceFitsOnBoard(unittest.TestCase):
    ...
    def test_piece_fits1(self) -> None:
    ...
    def test_piece_fits2(self) -> None:
        form = Form({GamePosition(0, 0), GamePosition(1, 0), GamePosition(1, 1)})
        piece = Piece(Color.green, form)
        board = GameBoard(
            {
    ...
    def test_piece_fits3(self) -> None:
        form = Form({GamePosition(0, 0), GamePosition(1, 0), GamePosition(1, 1)})
        piece = Piece(Color.green, form)
        board = GameBoard(
            {
    ...
    def test_piece_does_not_fit(self) -> None:
        form = Form(
            {
                GamePosition(0, 0),
                GamePosition(1, 0),
    ...
    def test_exact_fit_but_turned_form(self) -> None:
        form = Form({GamePosition(0, 0), GamePosition(0, 1), GamePosition(0, 2)})
        piece = Piece(Color.blue, form)
        board = GameBoard({GamePosition(0, 0), GamePosition(1, 0), GamePosition(2, 0)})
        self.assertTrue(piece.fits_on_board(board))

    def test_piece_unfits_multiple_times(self) -> None:
        form = Form({GamePosition(0, 0), GamePosition(1, 0), GamePosition(1, 1)})
        piece = Piece(Color.green, form)
        board = GameBoard(
    ...
    def test_piece_fits_multiple_times(self) -> None:
        form = Form({GamePosition(0, 0), GamePosition(1, 0), GamePosition(1, 1)})
        piece = Piece(Color.green, form)
...
class TestPiece(unittest.TestCase):
    ...
    def test_invalid_mirror_axis(self) -> None:
        with self.assertRaises(ValueError):
            self.piece.mirror("z")
