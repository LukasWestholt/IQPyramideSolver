...
class TestPieceFitsOnBoard(unittest.TestCase):
    ...
    def test_piece_fits2(self):
        ...
        # 90degres
        it = Form({GamePosition(x=0, y=1), GamePosition(x=-1, y=1), GamePosition(x=0, y=0)})
        assert it == piece.rotate_piece(90)
        ...
        for anchor in board:
            ...
            dx = anchor.x - min(pos.x for pos in it.position_set)
            dy = anchor.y - min(pos.y for pos in it.position_set)
    ...
    def test_empty_board(self):
        ...
        self.assertFalse(piece.fits_on_board(set()))

    def test_exact_fit(self):
        form = Form({GamePosition(0, 0), GamePosition(1, 0), GamePosition(2, 0)})
        piece = Piece(3, Color.blue, form)
        board = {GamePosition(0, 0), GamePosition(1, 0), GamePosition(2, 0)}
        self.assertTrue(piece.fits_on_board(board))
    ...
    def test_piece_unfits_multiple_times(self):
        ...
        new_boards = list(piece.delete_from_board(board))
        self.assertEqual(4, len(new_boards))
        self.assertFalse(piece.fits_on_board(new_boards[0]))
        self.assertFalse(piece.fits_on_board(new_boards[1]))
        self.assertFalse(piece.fits_on_board(new_boards[2]))
        self.assertFalse(piece.fits_on_board(new_boards[3]))

    # def test_piece_fits_multiple_times(self):
    #     form = Form({GamePosition(0, 0), GamePosition(1, 0), GamePosition(1, 1)})
    #     piece = Piece(3, Color.green, form)
    #     new_boards = list(piece.delete_from_board(self.board))
